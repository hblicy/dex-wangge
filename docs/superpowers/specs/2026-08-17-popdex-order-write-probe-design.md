...
- 下单和撤单交易哈希。
- 最后更新时间、脱敏错误摘要，以及人工撤单后由链上完成订单恢复时的 `recoveredFromChain` 布尔标记。

不保存 Agent 私钥、签名原文、原始交易或钱包认证数据。模拟、签名或广播前预检失败时，只允许清理仍处于 `PREPARED` 且没有广播证据的记录；广播后任何阶段都必须保留。探针启动时若发现未完成记录，必须拒绝新下单并进入只读恢复检查，显示下一步人工操作，不自动重发或撤单。

`--resume` 找到活动订单时只提示用户人工撤单，不执行写操作；后续若链上完成订单证明该订单零成交、剩余量为零且取消量等于原数量，可以设置 `recoveredFromChain=true` 并完成恢复清理，不得为人工撤单伪造 `cancelTxHash`。任何成交或字段冲突都必须保留记录并停止。
