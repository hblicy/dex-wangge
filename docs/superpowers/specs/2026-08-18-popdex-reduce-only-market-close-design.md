...
主网 Bot Agent 验收发现，只减仓市价单提交给 `placeOrder` 的 `price` 固定为
`0`，但成功 `OrderCreate` 事件中的 `price` 是 PopDEX 写入的实际执行参考价，
不是原始 calldata 的 `0`。因此回执验证必须明确区分两种订单：

- 普通限价单继续要求事件 `price` 与提交的 `priceWad` 完全一致；
- 只减仓市价平仓要求事件 `price` 为严格正 WAD 整数，不与提交参数 `0` 比较；
- 两种订单的账户、`symbolId`、`clientOrderId` 和 `qtyWad` 均继续完全匹配；
- 市价规则只能由平仓调用方显式选择，不能根据 `priceWad=0` 隐式放宽；
- 零值、负值、非整数或未知价格校验模式必须立即失败。

成功回执解析后仍必须进入 `CLOSE_SETTLING`，并以 REST 成交、活动订单为零和
链上空仓三组事实完成最终确认。事件价格为正只证明回执格式合法，不能单独证明
平仓完成。
...
11. 限价回执仍严格匹配提交价；市价平仓回执接受严格正的执行参考价，并拒绝
    零值、负值以及调用方未显式声明的宽松验证。
